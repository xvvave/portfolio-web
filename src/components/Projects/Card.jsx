...
const Card = ({
  ...
  const sizeClasses = {
    small: "min-h-[160px]",
    medium: "min-h-[220px]",
    large: "min-h-[280px] md:min-h-[336px]",
  }

  const titleClasses = {
    small: "text-xs sm:text-sm font-medium",
    medium: "text-sm sm:text-lg font-semibold",
  ...
  return (
    ...
    >
      ...
      <div className="relative h-full w-full flex items-center justify-center">
        <button
          className={`bg-black bg-opacity-70 rounded-md ${buttonSizes[size]} transform group-hover:scale-110 transition-transform`}
        >
          <svg
            className={`${iconSizes[size]} text-[#E6DADA] opacity-80`}
            viewBox="0 0 24 24"
            fill="none"
      ...
      <div className="absolute bottom-2 sm:bottom-3 left-2 sm:left-3 right-2 sm:right-3 flex flex-col">
        <h3 className={`${titleClasses[size]} text-[#E6DADA] line-clamp-1`}>{project.title}</h3>

        {showDescription && size === "large" && (
          <p className="text-[#E6DADA] text-opacity-80 text-xs sm:text-sm mt-1 line-clamp-2 hidden sm:block">
            {project.description}
          </p>
        )}
        ...
        {showCategory && (
          <div className={`mt-1 ${size === "small" ? "self-start" : ""}`}>
            <span
              className={`
              bg-[#E6DADA] bg-opacity-20 text-[#E6DADA] px-1 py-0.5 sm:px-2 sm:py-1 rounded-md text-[10px] sm:text-xs inline-block
              ${size === "small" ? "text-[10px] sm:text-xs px-1 py-0.5 bg-gray-200 bg-opacity-80 text-[#0E1011]" : ""}
            `}
            >
              {project.category}
            </span>
